...
def unpack_images(raw_packets):
    ...
    for packet in raw_packets:
        ...
        if len(packet)!=0:
            ...
            other_packets += 1
    print "correct: ", correct_packets, "special: ", special_packets, "other: ", other_packets
    #raw_input()
    ...
    print 'trying to get useful images out of packets'
    images=[]
    counter=0
    image=[]
    counter2=0
    # width * height / 4 / chunk size
    # 720 * 480 / 4 / 240
    #for small_packet in small_packets:
    #    if small_packet[1]==counter:
    #        print "Adding packet to image", small_packet[0], ":", small_packet[1], counter, counter2
    #        image.append(small_packet)
    #        counter+=1
    #        if counter==360:
    #            print "OMG ADDING THE THING TO THE STACK"
    #            counter=0
    #            images.append(image)
    #            image=[]
    #    else:
    #        print "Counter incorrect, resetting", small_packet[0], ":", small_packet[1], counter, counter2
    #        
    #        counter=330
    #        counter = small_packet[1] + 1
    #    counter2+=1

    real_counter = 0
    for small_packet in small_packets:
        n_img, n_packet, n_toggle, n_data = small_packet
        #print "n_img", n_img, "n_packet", n_packet, "n_toggle", n_toggle, "len(data)", len(n_data)
        while counter < n_packet:
            counter += 1
            image.append((n_img, counter, n_toggle, "\0" * 960))
        image.append(small_packet)
        counter += 1
        real_counter += 1
        
        if image[0][0] != n_img:
            print "n_img", n_img, "real lines", real_counter
            while counter < 360:
                counter += 1
                image.append((n_img, counter, n_toggle, "\0" * 960))
            images.append(image)
    ...
    for img in images:
        ...
        n_img=img[0][0]
        n_toggle=img[0][2]
        print "Image", n_img, n_toggle, len(img)
        #raw_input()
        for row in img:
            #print len(row[3]), "row length"
            full_image+=row[3]
        #print " 2 *len(full_image)/3" , 2*len(full_image)/3
        #raw_input()
        new_n_cols=(3*len(img[0][3])/2)
        #new_n_cols=(2*len(img[0][3])/3)
        #new_n_cols = 960
...
def main():
    ...
    #exit()
    #print utv.image
    #im = Image.frombuffer("L", (720, 480), "".join(utv.image))
    #im.show()
    ...
    #_, size = vis_images_final[0]
    #base = Image.new("RGB", (size[0]/3, size[1]))
    print "visualizing"
